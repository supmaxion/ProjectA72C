...
function makeNebulaTexture(color1 = '#4466ff', color2 = '#aa00ff', size = 256) {
  ...
  // Több réteg radial gradient → gázfelhő hatás
  const layers = [
  ...
  // Apró csillagpontok a nebulában
...
function makeGalaxyTexture(size = 256) {
  ...
  for (let arm = 0; arm < 3; arm++) {
    ...
    for (let i = 0; i < 120; i++) {
      ...
      const py = Math.sin(angle) * radius * 0.45; // lapított
...
function makeStarClusterTexture(size = 256) {
  ...
  // Sok apró csillag, Gauss-szerű sűrűséggel középen
  for (let i = 0; i < 200; i++) {
...
export class BackgroundObject {
   ...
   */
  constructor(scene, position, type = 'nebula') {
    this.scene = scene;
    this.type = type;
    this._time = Math.random() * Math.PI * 2; // pulsar fázis offset
    ...
    this.sprite.renderOrder = -1; // háttér mögé
...
// ─── Factory: random szórás a pályán ─────────────────────────────────────────

/**
 * Létrehoz N darab BackgroundObject-et random pozícióban.
 * @param {THREE.Scene} scene
 * @returns {BackgroundObject[]}
 */
export function spawnBackgroundObjects(scene) {
  const objects = [];
  const { spawnRadius, counts } = CFG;

  const typeList = [
    ...Array(counts.nebula).fill('nebula'),
    ...Array(counts.galaxy).fill('galaxy'),
    ...Array(counts.starCluster).fill('starCluster'),
    ...Array(counts.pulsar).fill('pulsar'),
  ];

  for (const type of typeList) {
    // Gömb felületén random pont (nem clustered)
    const theta = Math.random() * Math.PI * 2;
    const phi = Math.acos(2 * Math.random() - 1);
    const r = spawnRadius.min + Math.random() * (spawnRadius.max - spawnRadius.min);
    ...
    objects.push(new BackgroundObject(scene, pos, type));
  }
